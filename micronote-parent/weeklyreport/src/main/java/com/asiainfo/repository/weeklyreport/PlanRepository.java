...
//    @Query(name = "select p.plan_id,p.content,p.start_date,p.end_date,p.state from plan p where p.end_date>?1 and p.start_date<?2")
//    public List<Plan> findWeeklyPlans(@Param("beginDate") long beginDate,@Param("endDate") long endDate);
    
    public List<Plan> findByEndDateGreaterThanAndStartDateLessThan(@Param("beginDate") long beginDate, @Param("endDate") long endDate);
}
